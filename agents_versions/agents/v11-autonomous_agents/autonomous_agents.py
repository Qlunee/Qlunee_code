...
def claim_task(task_id: str, owner: str) -> str:
    with _claim_lock:
        ...
        if task.get("owner"):
            existed_owner = task.get("owner") or "someoneelse"
            return f"Error: Task {task_id} already claimed by {existed_owner}"
        if task.get("status") != "pending":
...
# ---teamate management with shutdown and plan approval protocols---
class TeammateManager:
    ...
    def _loop(self, name: str, role: str, prompt: str):
        ...
        sys_prompt = (
            ...
            "You are an autonomous agent. If you have no specific tasks assigned, "
            "immediately call the 'idle' tool so you can check the task board for new work."        )
...
def _run_read(file_path: str, limit: Optional[int] = None) -> str:
    try:
...
def _run_write(file_path: str, content: str) -> str:
    try:
...
def _run_edit(file_path: str, old_content: str, new_content: str) -> str:
    try:
        fp = safe_path(file_path)
...
def handle_shutdown_request(teammate: str) -> str:
    req_id = str(uuid.uuid4())[:8]
    with _tracker_lock:
        shutdown_requests[req_id] = {"teammate": teammate, "status": "pending"}
    BUS.send("lead", teammate, "Please shut down gracefully.", "shutdown_request", {"requset_id": req_id})
    return f"Sent shutdown request to {teammate} (req_id: {req_id})"

def handle_plan_review(request_id: str, approve: bool, feedback: str = '') -> str:
    with _tracker_lock:
        req = plan_requests.get(request_id)
    if not req:
        return f"Error: No such plan request {request_id}"
...
def _check_shutdown_status(request_id: str) -> str:
    with _tracker_lock:
        return json.dumps(shutdown_requests.get(request_id, {"error": "not found"}), indent=2)

# --dispatch map: {tool_name: handler} --
TOOL_HANDLERS = {
    "bash":                             lambda **kwargs: _run_bash(kwargs["command"]),
    "read_file":                        lambda **kwargs: _run_read(kwargs["file_path"], kwargs.get("limit")),
    "write_file":                       lambda **kwargs: _run_write(kwargs["file_path"], kwargs["content"]),
    "edit_file":                        lambda **kwargs: _run_edit(kwargs["file_path"], kwargs["old_content"], kwargs["new_content"]),
...
if __name__ == "__main__":
    ...
    while True:
        try:
            query = input("\033[36mQlunee_code_v11 >> \033[0m")
